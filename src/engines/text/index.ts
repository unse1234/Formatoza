...
function textOut(
  ...
  return {
    name: outputFileName(sourceName, out.ext),
    mimeType: out.mime,
...
async function convertOne(
  ...
  if (encoding === 'windows-1252' && from !== 'base64' && from !== 'urlencoded')
    warnings.push(
      warning('The file was not valid UTF-8 and was read as Windows-1252. The output is UTF-8.'),
    );
  if (!text.trim() && from !== 'txt')
    throw new ConversionError('EMPTY_INPUT', 'The input is empty.');
  ...
  switch (key) {
      ...
      return {
        outputs: [
          textOut(
            ...
            markdownToHtml(text, {
              gfm: o['gfm'] !== false,
              breaks: o['breaks'] === true,
              fullDocument: o['fullDocument'] === true,
            }),
          ),
        ],
        warnings,
    ...
    case 'markdown>txt': {
      ...
      return {
        outputs: [
          textOut(file.name, to, markdownToText(text, { keepLinks: o['keepLinks'] === true })),
        ],
        warnings,
      };
    }
    ...
    case 'html>txt': {
      if (typeof DOMParser === 'undefined')
        throw new ConversionError('BROWSER_UNSUPPORTED', 'This conversion needs a browser DOM.');
      const html = await import('./html');
      const body =
        key === 'html>txt'
          ? html.htmlToText(text, {
              keepLinks: o['keepLinks'] === true,
              keepImagesAlt: o['keepImagesAlt'] === true,
            })
          : html.htmlToMarkdown(text, {
    ...
    case 'txt>base64': {
      ...
      return {
        outputs: [
          textOut(file.name, to, body, {
            'Input bytes': formatBytes(new TextEncoder().encode(text).length),
            'Output chars': body.length.toLocaleString('en-US'),
          }),
        ],
        warnings,
      };
    ...
    case 'json>base64': {
      ...
      warnings.push(...jw);
      const json =
        o['minify'] !== false ? JSON.stringify(value) : text.replace(/^\ufeff/, '').trim();
      return {
        outputs: [
          textOut(
            file.name,
            to,
            encodeBase64(json, { urlSafe: o['urlSafe'] === true, wrap: false }),
          ),
        ],
        warnings,
      };
    ...
    case 'base64>json': {
      ...
      if (JWT.test(trimmed)) {
        ...
        const parse = (seg: string, label: string) => {
          ...
          if (decoded === null)
            throw new ConversionError('MALFORMED_INPUT', `The JWT ${label} is not text.`);
          return parseJson(decoded, { allowJsonLines: false }).value;
        };
        const value = { header: parse(h!, 'header'), payload: parse(p!, 'payload') };
        warnings.push(
          warning(
            'This looks like a JSON Web Token: its header and payload were decoded. The signature was NOT verified.',
          ),
        );
        return {
          outputs: [textOut(file.name, to, `${JSON.stringify(value, null, indent)}\n`)],
          warnings,
        };
      }
      ...
      if (asText === null)
        throw new ConversionError(
          'MALFORMED_INPUT',
          'The Base64 decodes to binary data, not JSON text. Try Base64 to Text to download it as a file.',
        );
      const { value } = parseJson(asText, { allowJsonLines: false });
      if (decoded.paddingAdded)
        warnings.push(warning('Missing “=” padding was added automatically.'));
      return {
        outputs: [textOut(file.name, to, `${JSON.stringify(value, null, indent)}\n`)],
        warnings,
      };
    ...
    case 'base64>txt': {
      ...
      if (decoded.urlSafe)
        warnings.push(warning('URL-safe Base64 (- and _) was detected and decoded.'));
      if (decoded.paddingAdded)
        warnings.push(warning('Missing “=” padding was added automatically.'));
      const asText = bytesAsText(decoded.bytes);
      if (asText !== null)
      ...
      warnings.push(
        warning(
          `This Base64 is not text — it decodes to ${kind?.label ?? 'binary data'} (${formatBytes(decoded.bytes.length)}). Download it as a file below.`,
        ),
      );
      return {
        outputs: [
          {
            name: outputFileName(file.name === 'pasted.txt' ? 'decoded' : file.name, ext),
            mimeType: mime,
    ...
    case 'txt>urlencoded': {
      ...
      if (/%[0-9a-f]{2}/i.test(text))
        warnings.push(
          warning(
            'The input already contains percent-encoded sequences; encoding again turns % into %25. Use the URL Decoder first if this was not intended.',
          ),
        );
      return {
        outputs: [
    ...
    case 'urlencoded>txt': {
      const r = urlDecode(text, {
        plusAsSpace: o['plusAsSpace'] !== false,
        repeat: o['repeat'] === true,
      });
      if (r.invalidSequences)
        warnings.push(
          warning(
            `${r.invalidSequences} sequence(s) were not valid UTF-8 and were decoded as Windows-1252.`,
          ),
        );
      if (r.passes > 1)
        warnings.push(
          warning(`The text was encoded ${r.passes} times; it was decoded ${r.passes} times.`),
        );
      if (o['repeat'] !== true && /%(25|2[0-9a-f]|3[a-f]|40|5[bd])/i.test(r.text))
        warnings.push(
          warning(
            'The result still contains percent-encoded characters — the text may have been encoded twice. Turn on “Decode repeatedly”.',
          ),
        );
