...
function rowList(rows: number[]): string {
  ...
  return rows.length > MAX_REPORTED_ROWS
    ? `${shown} and ${rows.length - MAX_REPORTED_ROWS} more`
    : shown;
}
...
export function parseDelimited(text: string, opts: ParseDelimitedOptions): ParsedDelimited {
  ...
  const data = res.data;
  if (data.length === 0)
    throw new ConversionError('EMPTY_INPUT', 'No rows were found in the input.');
  ...
  if (quoteErrors.length) {
    ...
    warnings.push(
      warning(
        `Unbalanced quotes near row ${rowList(rows)}. Everything after an unclosed quote is read as one field — check the result.`,
      ),
    );
  }
  if (
    opts.delimiter === 'auto' &&
    res.meta.delimiter &&
    data.some((r) => r.length > 1) === false &&
    input.includes('\n')
  )
    warnings.push(
      warning(
        'Only one column was found. If your data uses another separator, pick it under Settings → Input delimiter.',
      ),
    );

  const width = opts.header ? data[0]!.length : Math.max(...data.map((r) => r.length));
  const headerRow = opts.header
    ? data[0]!
  ...
  if (short.length)
    warnings.push(
      warning(
        `Row ${rowList(short)} had fewer fields than the header; missing values were left empty.`,
      ),
    );
  if (long.length)
    warnings.push(
      warning(
        `Row ${rowList(long)} had more fields than the header; extra values were kept in added columns (column_${width + 1}…).`,
      ),
    );

  const columns = normalizeHeaders(
    [...headerRow, ...Array.from({ length: maxLen - width }, () => '')].map((h, i) =>
      i >= width ? `column_${i + 1}` : h,
    ),
  );
