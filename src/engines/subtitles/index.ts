...
export function prepareCues(input: Cue[], offsetMs: number): { cues: Cue[]; notes: string[] } {
  ...
  if (sorted.some((c, i) => c !== cues[i])) notes.push('Cues were out of order and have been sorted by start time.');
  cues = sorted;
  let fixed = 0;
  for (const c of cues)
    if (c.end <= c.start) {
  ...
  if (fixed) notes.push(`${fixed} cue(s) ended before they started; their end time was set to 1 s after the start.`);
  if (offsetMs) {
    ...
    if (cues.length < before) notes.push(`${before - cues.length} cue(s) fell before 0:00 after shifting and were removed.`);
  }
  return { cues, notes };
}
  ...
  async convert(input, rawOptions, ctx) {
    ...
    const result = await runPerFile(input.files, ctx, async (file) => {
      ...
      if (encoding === 'windows-1252')
        warnings.push(warning('The file was not valid UTF-8 and was read as Windows-1252 (Western European). The output is UTF-8.'));
      const parsed = parse(text, input.from);
      const { cues, notes } = prepareCues(parsed.cues, input.to === 'txt' ? 0 : Number(o['offsetMs'] ?? 0));
      ...
      switch (input.to) {
        ...
        case 'vtt':
          ...
          break;
        case 'ass': {
          const [w, h] = String(o['resolution'] ?? '1920x1080').split('x').map(Number);
          if (cues.some((c) => c.start % 10 !== 0 || c.end % 10 !== 0))
            warnings.push(warning('ASS stores time in hundredths of a second, so timings were rounded to the nearest 10 ms.'));
          body = writeAss(cues, { fontName: String(o['fontName']), fontSize: Number(o['fontSize']), width: w ?? 1920, height: h ?? 1080 });
          break;
        }
        default:
          body = writeTxt(cues, { timestamps: o['timestamps'] === true, paragraphs: o['paragraphs'] !== false, dedupe: o['dedupe'] !== false });
      }
      return {
        outputs: [
          {
