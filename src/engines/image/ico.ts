...
export function parseIco(bytes: Uint8Array): IcoEntry[] {
  ...
  if (reserved !== 0 || (type !== 1 && type !== 2)) throw new ConversionError('MALFORMED_INPUT', 'This is not a valid .ico file.');
  if (count === 0) throw new ConversionError('EMPTY_INPUT', 'The icon file contains no images.');
  if (6 + count * 16 > bytes.length) throw new ConversionError('MALFORMED_INPUT', 'The icon directory is truncated.');
  ...
  for (let i = 0; i < count; i++) {
    ...
    const isPng = bytes[offset] === 0x89 && bytes[offset + 1] === 0x50 && bytes[offset + 2] === 0x4e && bytes[offset + 3] === 0x47;
    ...
    entries.push({ width, height, bitCount: dv.getUint16(p + 6, true) || (isPng ? 32 : 0), offset, size, isPng });
  }
  if (!entries.length) throw new ConversionError('MALFORMED_INPUT', 'None of the images inside the icon could be read.');
  return entries;
}

/** Largest entry, preferring higher bit depth on ties. */
export function largestEntry(entries: IcoEntry[]): IcoEntry {
  return [...entries].sort((a, b) => b.width * b.height - a.width * a.height || b.bitCount - a.bitCount)[0]!;
}
