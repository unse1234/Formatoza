...
function loadImage(url: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => reject(new ConversionError('MALFORMED_INPUT', 'The SVG could not be rendered. It may be invalid or rely on unsupported features.'));
    img.src = url;
  });
}
...
async function decodeSvg(file: Blob, o: DecodeOptions): Promise<DecodeResult> {
  ...
  if (!root || root.nodeName.toLowerCase() !== 'svg' || doc.getElementsByTagName('parsererror').length)
    throw new ConversionError('MALFORMED_INPUT', 'This is not a valid SVG file (the XML could not be parsed).');
  const warnings: ConversionIssue[] = [];
  const size = svgIntrinsicSize({ width: root.getAttribute('width'), height: root.getAttribute('height'), viewBox: root.getAttribute('viewBox') });
  if (!root.getAttribute('width') && !root.getAttribute('height') && !root.getAttribute('viewBox'))
    warnings.push(warning('The SVG declares no size; it was rendered at 300 × 150, the browser default. Use a custom width if that is wrong.'));
  if (/<image[\s>]|<use[^>]+href=["'](?!#)|@import|url\(\s*["']?https?:/i.test(text))
    warnings.push(warning('External images, fonts or files referenced by the SVG are not loaded (for security), so they will be missing.'));
  if (/<text[\s>]/i.test(text))
    warnings.push(warning('Text in the SVG is drawn with fonts installed on this device; if the original font is missing, a fallback is used.'));

  const scale = o.svgScale === 'width' ? Math.max(16, o.svgWidth ?? 1024) / size.width : Number(o.svgScale ?? 1) || 1;
...
async function decodeHeic(file: Blob): Promise<DecodeResult> {
  ...
  if (native) return { frames: [{ source: native, width: native.width, height: native.height, suffix: '' }], warnings: [] };
  try {
    const { heicTo } = await import('heic-to/csp');
    const bitmap = await heicTo({ blob: file, type: 'bitmap' });
    return { frames: [{ source: bitmap, width: bitmap.width, height: bitmap.height, suffix: '' }], warnings: [] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    throw new ConversionError('MALFORMED_INPUT', `This HEIC image could not be decoded${msg ? ` (${msg.slice(0, 120)})` : ''}. It may be damaged or use an unsupported HEIF variant.`);
  }
}
...
async function decodeTiff(file: Blob, o: DecodeOptions): Promise<DecodeResult> {
  ...
  if (!ifds.length) {
    ...
    if (native) return { frames: [{ source: native, width: native.width, height: native.height, suffix: '' }], warnings: [] };
    throw new ConversionError('MALFORMED_INPUT', 'This TIFF file could not be read. It may be damaged or use an unsupported compression.');
  }
  const warnings: ConversionIssue[] = [];
  const pages = o.tiffPages === 'first' ? ifds.slice(0, 1) : ifds;
  if (o.tiffPages === 'first' && ifds.length > 1) warnings.push(warning(`Only the first of ${ifds.length} pages was converted.`));
  ...
  for (const [i, ifd] of pages.entries()) {
    try {
      ...
      const bitmap = await bitmapFromRgba({ width, height, data: new Uint8ClampedArray(rgba.buffer, rgba.byteOffset, width * height * 4) });
      frames.push({ source: bitmap, width, height, suffix: pages.length > 1 ? `-page-${i + 1}` : '' });
    } catch {
      warnings.push(warning(`Page ${i + 1} uses a TIFF encoding that could not be decoded and was skipped.`));
    }
  }
  if (!frames.length) throw new ConversionError('MALFORMED_INPUT', 'None of the pages in this TIFF could be decoded.');
...
async function decodeIco(file: Blob, o: DecodeOptions): Promise<DecodeResult> {
  ...
  const chosen = o.icoEntries === 'all' ? [...entries].sort((a, b) => b.width - a.width) : [largestEntry(entries)];
  const frames: DecodedFrame[] = [];
  const warnings: ConversionIssue[] = [];
  ...
  for (const e of chosen) {
    try {
      ...
      const source = e.isPng ? await createImageBitmap(new Blob([data], { type: 'image/png' })) : await bitmapFromRgba(decodeDib(data, true));
      ...
      warnings.push(warning(`The ${e.width}×${e.height} icon image could not be decoded and was skipped.`));
    }
  }
  if (!frames.length) throw new ConversionError('MALFORMED_INPUT', 'No image inside this icon could be decoded.');
  if (o.icoEntries !== 'all' && entries.length > 1)
    warnings.push(warning(`The icon contains ${entries.length} sizes; the largest (${frames[0]!.width}×${frames[0]!.height}) was converted. Choose “Every size” in Settings to export all.`));
  return { frames, warnings };
}

const LABEL: Partial<Record<FormatId, string>> = { avif: 'AVIF', webp: 'WebP', jpg: 'JPEG', png: 'PNG', gif: 'GIF', bmp: 'BMP' };
...
async function decodeNative(file: Blob, from: FormatId): Promise<DecodeResult> {
  ...
  if (from === 'gif') {
    ...
    if (frames > 1) warnings.push(warning(`This GIF is animated (${frames} frames); the first frame was converted.`));
  }
  const bitmap = await nativeBitmap(file);
  if (bitmap) return { frames: [{ source: bitmap, width: bitmap.width, height: bitmap.height, suffix: '' }], warnings };
  ...
  if (from === 'avif' && (actual === 'avif' || actual === 'unknown'))
    throw new ConversionError('BROWSER_UNSUPPORTED', 'This browser cannot decode AVIF images. Use a current version of Chrome, Edge, Firefox (93+) or Safari (16.4+).');
  throw new ConversionError('MALFORMED_INPUT', `The ${LABEL[from] ?? from.toUpperCase()} image could not be decoded — the file may be damaged or incomplete.`);
}

/** Detects the real format from the bytes and routes to the right decoder. */
export async function decodeImage(file: Blob, from: FormatId, o: DecodeOptions = {}): Promise<DecodeResult> {
  const head = new Uint8Array(await file.slice(0, 1024).arrayBuffer());
  const actual = sniffBytes(head);
  const extra: ConversionIssue[] = [];
  let route: FormatId = from;
  const ROUTABLE: FormatId[] = ['heic', 'avif', 'webp', 'jpg', 'png', 'svg', 'gif', 'bmp', 'tiff', 'ico'];
  if (actual !== 'unknown' && actual !== from && ROUTABLE.includes(actual as FormatId)) {
    extra.push(warning(`This file is actually ${actual.toUpperCase()} data despite its name; it was decoded as ${actual.toUpperCase()}.`));
    route = actual as FormatId;
  } else if (actual === 'pdf' || actual === 'zip') {
    throw new ConversionError('UNSUPPORTED_FORMAT', `This file is a ${actual.toUpperCase()}, not an image.`);
  }
  let r: DecodeResult;
  switch (route) {
    case 'svg':
