...
export function decodeDib(bytes: Uint8Array, isIcon: boolean, pixelOffset?: number): RgbaImage {
  if (bytes.length < 40)
    throw new ConversionError('MALFORMED_INPUT', 'Bitmap header is truncated.');
  const dv = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  const headerSize = dv.getUint32(0, true);
  if (headerSize < 40)
    throw new ConversionError('UNSUPPORTED_FORMAT', 'Old OS/2 bitmap headers are not supported.');
  ...
  if (width <= 0 || height <= 0 || width * height > MAX_PIXELS)
    throw new ConversionError('MALFORMED_INPUT', 'Invalid bitmap dimensions.');
  if (![0, 3, 6].includes(compression))
    throw new ConversionError(
      'UNSUPPORTED_FORMAT',
      'Compressed (RLE/JPEG/PNG) bitmaps are not supported by the fallback decoder.',
    );
  ...
  const maskBytes =
    compression === 3 && headerSize === 40 ? 12 : compression === 6 && headerSize === 40 ? 16 : 0;
  ...
  if (dataStart + rowSize * height > bytes.length)
    throw new ConversionError('MALFORMED_INPUT', 'Bitmap pixel data is truncated.');
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (bpp === 32 || bpp === 16) {
        const v =
          bpp === 32 ? dv.getUint32(srcRow + x * 4, true) : dv.getUint16(srcRow + x * 2, true);
        ...
        throw new ConversionError('UNSUPPORTED_FORMAT', `${bpp}-bit bitmaps are not supported.`);
      out[o] = r;
      out[o + 1] = g;
      out[o + 2] = b;
      out[o + 3] = a;
  ...
  if (bpp === 32 && compression === 0 && !anyAlpha)
  ...
  if (isIcon && !(bpp === 32 && anyAlpha)) {
    ...
    if (maskStart + maskRow * height <= bytes.length)
      for (let y = 0; y < height; y++) {
        ...
        for (let x = 0; x < width; x++)
          if ((bytes[row + (x >> 3)]! >> (7 - (x & 7))) & 1) out[(y * width + x) * 4 + 3] = 0;
      }
  }
...
export function decodeBmpFile(bytes: Uint8Array): RgbaImage {
  if (bytes.length < 54 || bytes[0] !== 0x42 || bytes[1] !== 0x4d)
    throw new ConversionError('MALFORMED_INPUT', 'Not a BMP file.');
  const dv = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
