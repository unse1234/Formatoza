...
export function parsePageRange(spec: string, pageCount: number): number[] {
  ...
  for (const part of s.split(/[,;\s]+/).filter(Boolean)) {
    ...
    if (m) {
      ...
      if (a < 1 || b < a) throw new ConversionError('MALFORMED_INPUT', `“${part}” is not a valid page range.`);
    ...
    } else throw new ConversionError('MALFORMED_INPUT', `“${part}” is not a page number or range (use e.g. 1-3, 5, 8-).`);
  }
  if (!pages.size) throw new ConversionError('MALFORMED_INPUT', `The selection “${s}” contains no page of this ${pageCount}-page document.`);
...
const PAGE_SIZES: Record<string, Box> = { a4: { width: 595.28, height: 841.89 }, letter: { width: 612, height: 792 } };
...
export function placeImage(px: Box, pageSize: string, orientation: string, margin: number): Placement {
  ...
  if (pageSize === 'fit' || !PAGE_SIZES[pageSize]) {
    ...
    w *= s;
    h *= s;
    return { page: { width: w + 2 * margin, height: h + 2 * margin }, x: margin, y: margin, width: w, height: h };
  }
