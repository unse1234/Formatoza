...
w('rotated.jpg', await base().flatten({ background: '#ffffff' }).jpeg({ quality: 92 }).withMetadata({ orientation: 6 }).toBuffer());
...
function multiPageTiff(pages, width, height) {
  const tags = (dataOffset, bpsOffset, next) => [
    [256, 3, 1, width], [257, 3, 1, height], [258, 3, 4, bpsOffset], [259, 3, 1, 1], [262, 3, 1, 2],
    [273, 4, 1, dataOffset], [277, 3, 1, 4], [278, 3, 1, height], [279, 4, 1, width * height * 4], [284, 3, 1, 1], [338, 3, 1, 2],
  ];
  ...
  pages.forEach((px, n) => {
    ...
    entries.forEach(([tag, type, count, value], i) => {
      const p = 2 + i * 12;
      ifd.writeUInt16LE(tag, p); ifd.writeUInt16LE(type, p + 2); ifd.writeUInt32LE(count, p + 4);
      if (type === 3 && count === 1) ifd.writeUInt16LE(value, p + 8); else ifd.writeUInt32LE(value, p + 8);
    });
...
for (let i = 0; i < page2.length; i += 4) { page2[i] = 255 - page2[i]; page2[i + 3] = 255; }
...
const framePngs = await Promise.all(frames.map((f) => sharp(f, { raw: { width: W, height: H, channels: 4 } }).png().toBuffer()));
w('animated.gif', await sharp(framePngs, { join: { animated: true } }).gif({ loop: 0, delay: [200, 200, 200] }).toBuffer());
...
function dibEntry(size) {
  ...
  for (let y = 0; y < size; y++)
    for (let x = 0; x < size; x++) {
      ...
      b[d] = 40; b[d + 1] = 90; b[d + 2] = 200; b[d + 3] = edge ? 0 : 255;
    }
  return b;
}
const icoPng = await sharp(rgba, { raw: { width: W, height: H, channels: 4 } }).resize(32, 32).png().toBuffer();
const entries = [{ size: 16, data: dibEntry(16), bpp: 32 }, { size: 32, data: icoPng, bpp: 32 }];
...
entries.forEach((e, i) => {
  const p = 6 + 16 * i;
  icoHead[p] = e.size; icoHead[p + 1] = e.size;
  icoHead.writeUInt16LE(1, p + 4); icoHead.writeUInt16LE(e.bpp, p + 6);
  icoHead.writeUInt32LE(e.data.length, p + 8); icoHead.writeUInt32LE(off, p + 12);
  off += e.data.length;
});
w('sample.ico', Buffer.concat([icoHead, ...entries.map((e) => e.data)]));

w('sample.svg', `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 48" width="64" height="48">
...
// text and an embedded PNG image.
{
  ...
  const li = (t) => `<w:p><w:pPr><w:pStyle w:val="ListParagraph"/><w:numPr><w:ilvl w:val="0"/><w:numId w:val="1"/></w:numPr></w:pPr>${run(t)}</w:p>`;
  const cell = (t, bold) => `<w:tc><w:tcPr><w:tcW w:w="2000" w:type="dxa"/></w:tcPr>${para(run(t, bold ? '<w:b/>' : ''))}</w:tc>`;
  ...
  const body = [
    ...
    para(run('This document tests ') + run('bold', '<w:b/>') + run(', ') + run('italic', '<w:i/>') + run(' and a ') +
      '<w:hyperlink r:id="rIdLink1"><w:r><w:rPr><w:rStyle w:val="Hyperlink"/></w:rPr><w:t>link</w:t></w:r></w:hyperlink>' + run('. Ünïcödé: café, 東京, 👋.')),
  ...
  const files = {
    ...
    'word/_rels/document.xml.rels': strToU8('<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rIdStyles" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/><Relationship Id="rIdNum" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/numbering" Target="numbering.xml"/><Relationship Id="rIdLink1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="https://example.com/" TargetMode="External"/><Relationship Id="rIdImg1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/chart.png"/></Relationships>'),
    'word/document.xml': strToU8(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document ${ns}><w:body>${body}<w:sectPr/></w:body></w:document>`),
    ...
    'word/numbering.xml': strToU8('<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:numbering xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:abstractNum w:abstractNumId="0"><w:lvl w:ilvl="0"><w:start w:val="1"/><w:numFmt w:val="bullet"/><w:lvlText w:val="•"/></w:lvl></w:abstractNum><w:num w:numId="1"><w:abstractNumId w:val="0"/></w:num></w:numbering>'),
...
w('legacy.doc', Buffer.concat([Buffer.from([0xd0, 0xcf, 0x11, 0xe0, 0xa1, 0xb1, 0x1a, 0xe1]), Buffer.alloc(504)]));
// Password-protected PDF (open password "secret"), via pypdf when available.
try {
  execFileSync('python3', ['-c', `from pypdf import PdfReader, PdfWriter
w = PdfWriter(clone_from=PdfReader("${OUT}/sample.pdf"))
w.encrypt(user_password="secret", owner_password="owner", algorithm="RC4-128")
w.write("${OUT}/encrypted.pdf")`], { stdio: 'inherit' });
} catch {
  console.warn('pypdf not available: encrypted.pdf was not regenerated.');
}

// ------------------------------------------------------------------ text formats
w('sample.csv', 'id,name,email,city,zip,active,score\n1,Ana Souza,ana@example.com,São Paulo,01310-100,true,9.5\n2,"Lee, Min-jun",minjun@example.com,서울,04524,false,10\n3,"Zoë ""Z"" Müller",zoe@example.com,Berlin,10115,true,1.50\n');
w('semicolon.csv', 'produkt;preis;menge\nÄpfel;1,99;3\nBirnen;2,49;10\n');
w('malformed.csv', 'a,b,c\n1,2,3\n4,5\n6,7,8,9\n"unclosed,10,11\n');
w('sample.tsv', 'sku\tproduct\tprice\tstock\nA-001\tDesk lamp\t39.90\t12\nA-002\tNotebook, dotted\t4.50\t240\n');
w('sample.json', JSON.stringify([
  { id: 1, name: 'Ana Souza', tags: ['admin', 'editor'], address: { city: 'São Paulo', zip: '01310-100' }, active: true },
  { id: 2, name: 'Lee Min-jun 👋', tags: [], address: { city: '서울', zip: '04524' }, active: false, note: null },
], null, 2) + '\n');
w('envelope.json', JSON.stringify({ meta: { page: 1 }, data: { items: [{ a: 1 }, { a: 2, b: 'x' }] } }) + '\n');
w('lines.jsonl', '{"a":1}\n{"a":2,"b":"two"}\n');
w('malformed.json', '{\n  "a": 1,\n  "b": [1, 2,],\n}\n');
...
w('entities.xml', '<?xml version="1.0"?>\n<!DOCTYPE lolz [<!ENTITY lol "lol"><!ENTITY lol2 "&lol;&lol;&lol;">]>\n<lolz>&lol2;</lolz>\n');
w('sample.yaml', `# Service configuration
service:
  name: checkout
  replicas: 3
  enabled: yes
...
w('sample.md', `# Release notes\n\nVersion **2.4** adds _offline mode_ &amp; fixes bugs. Ünïcödé 👋.\n\n## Highlights\n\n- Works offline\n- [x] Faster start-up\n  1. nested\n\n| Platform | Status |\n|---|---|\n| Web | ✅ |\n\n\`\`\`js\nconsole.log('hi');\n\`\`\`\n\nSee [the blog](https://example.com/blog).\n`);
w('sample.html', `<!doctype html><html><head><title>t</title><style>p{color:red}</style><script>alert(1)</script></head><body>
